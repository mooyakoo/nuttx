						    ...
						    unsigned int event_len)
{
	...
	case NRF_WIFI_UMAC_EVENT_CMD_STATUS:
		...
		/* If TRIGGER_SCAN (cmd_id=0) fails (non-zero status), the RPU
		 * will not send SCAN_DONE.  Call scan_done_callbk_fn so the
		 * driver can unblock its scan semaphore and retry or proceed.
		 */
		if (cmd_status->cmd_id == NRF_WIFI_UMAC_CMD_TRIGGER_SCAN &&
		    cmd_status->cmd_status != 0) {
			if (callbk_fns->scan_done_callbk_fn) {
				callbk_fns->scan_done_callbk_fn(
					vif_ctx->os_vif_ctx, NULL, 0);
			}
		}
		/* If AUTHENTICATE (cmd_id=2) fails (non-zero status), the RPU
		 * will not send NRF_WIFI_UMAC_EVENT_AUTHENTICATE.  Call
		 * auth_resp_callbk_fn with NULL so the driver can unblock its
		 * auth semaphore and retry SCAN_CONNECT + auth.
		 */
		if (cmd_status->cmd_id == NRF_WIFI_UMAC_CMD_AUTHENTICATE &&
		    cmd_status->cmd_status != 0) {
			nrf_wifi_osal_log_err("%s: auth CMD_STATUS error %d"
					      " — posting NULL auth callback",
					      __func__,
					      cmd_status->cmd_status);
			if (callbk_fns->auth_resp_callbk_fn) {
