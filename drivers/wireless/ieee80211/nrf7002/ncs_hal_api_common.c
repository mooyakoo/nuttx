...
enum nrf_wifi_status nrf_wifi_hal_irq_handler(void *data)
{
	...
	/* NuttX port: this handler already runs on the HPWORK thread (the HOSTIRQ
	 * ISR defers to HPWORK via work_queue), not in hard-IRQ context.  The NCS
	 * event_tasklet exists to move event processing out of hard IRQ on
	 * Linux/Zephyr; here it just adds a second HPWORK scheduling round-trip
	 * (per-IRQ latency) that throttles TX-done replenishment and TX throughput.
	 * We already hold lock_rx (the same lock event_tasklet_fn takes), so drain
	 * and process the event queue inline instead of re-deferring.
	 */

	hal_rpu_eventq_process(hal_dev_ctx);
